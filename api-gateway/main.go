...
func main() {
	...
	brandsRoutes := r.Group("/brands")
	{
		...
		brandsRoutes.GET("/config-brands/brand", reverseProxy("http://localhost:8081"))
	...
	phoneRoutes := r.Group("/phones")
	{
		...
		phoneRoutes.GET("/storages", reverseProxy("http://localhost:8086"))
		...
		phoneRoutes.GET("/view-phone-with-deductions", reverseProxy("http://localhost:8086"))
